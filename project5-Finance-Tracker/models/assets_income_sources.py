...
def get_all_assets_income():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    
    base_query = """SELECT id, name, type, details FROM assets;"""
...
def get_assets_income_sources_by_filter(filter_option):
    
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    
    base_query = """SELECT name, type, details FROM assets"""
...
def insert_asset(name, type, details):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
